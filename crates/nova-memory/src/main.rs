//! Nova Memory Daemon - HTTP proxy for transparent LLM memory injection
...
use nova_memory::error::Result;
use nova_memory::memory::ingestion::IngestionPipeline;
use nova_memory::proxy::ProxyServer;
use nova_memory::router::MemoryRouter;
use nova_memory::storage::LanceStore;

/// Nova Memory - Transparent HTTP proxy that gives your LLM long-term memory
#[derive(Parser)]
#[command(name = "nova-memory")]
...
fn init_logging() {
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("info,nova_memory=debug"));
...
fn load_config(config_path: Option<PathBuf>) -> Result<Config> {
    if let Some(path) = config_path {
        ...
        let content = std::fs::read_to_string(&path).map_err(|e| {
            nova_memory::NovaError::Config(format!(
        ...
        let default_paths = [
            dirs::home_dir().map(|h| h.join(".nova-memory").join("config.toml")),
            dirs::config_dir().map(|c| c.join("nova-memory").join("config.toml")),
...
async fn serve(config_path: Option<PathBuf>) -> Result<()> {
    tracing::info!("Starting Nova Memory daemon");
    ...
    std::fs::create_dir_all(data_dir).map_err(|e| {
        nova_memory::NovaError::Storage(format!(
    ...
    tracing::info!("Nova Memory daemon stopped");
    Ok(())
}
