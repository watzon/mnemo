...
impl UpstreamTarget {
    ...
    /// * `Err(NovaError::Config)` - Invalid URL or unsupported scheme
    ///
    /// # Examples
    /// ```
    /// # use nova_memory::proxy::UpstreamTarget;
    ...
    pub fn from_path(path: &str, query: Option<&str>) -> Result<Self> {
        ...
        let url_str = path
            .strip_prefix("/p/")
            .ok_or_else(|| NovaError::Config(format!("Invalid passthrough path: {path}")))?;
        ...
        let mut url = Url::parse(&normalized)
            .map_err(|e| NovaError::Config(format!("Invalid URL '{normalized}': {e}")))?;
        ...
        let host = url
            .host_str()
            .ok_or_else(|| NovaError::Config(format!("URL '{normalized}' has no host")))?
            .to_string();
        ...
        if url.username() != "" || url.password().is_some() {
            ...
            url.set_username("")
                .map_err(|_| NovaError::Config("Failed to strip username from URL".to_string()))?;
            url.set_password(None)
                .map_err(|_| NovaError::Config("Failed to strip password from URL".to_string()))?;
        }
