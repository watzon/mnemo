...
pub fn format_memory_block(memories: &[RetrievedMemory]) -> String {
    ...
    let mut block = String::from("<nova-memories>\n");
    for rm in memories {
...
pub fn truncate_to_budget(memories: &[RetrievedMemory], max_tokens: usize) -> Vec<RetrievedMemory> {
    ...
    // "<nova-memories>\n" + "</nova-memories>" = ~35 chars = ~9 tokens
...
///
/// # Errors
/// Returns `NovaError::Proxy` if the request format is invalid.
pub fn inject_memories(
    ...
    let messages = request_body
        ...
        .ok_or_else(|| NovaError::Proxy("Missing or invalid 'messages' array".into()))?;
...
mod tests {
    ...
    fn test_format_memory_block_single_memory() {
        ...
        assert!(block.contains("<nova-memories>"));
        assert!(block.contains("</nova-memories>"));
    ...
    fn test_inject_missing_messages_returns_error() {
        ...
        assert!(matches!(result.unwrap_err(), NovaError::Proxy(_)));
    }
    ...
    fn test_inject_into_empty_system_message() {
        ...
        assert!(system_content.starts_with("<nova-memories>"));
    }
