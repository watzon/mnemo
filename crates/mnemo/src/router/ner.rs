...
impl NerModel {
    ...
    pub fn with_cache_dir(_cache_dir: Option<PathBuf>) -> Result<Self, MnemoError> {
        let device = Device::Cpu;

        let api =
            Api::new().map_err(|e| MnemoError::Router(format!("Failed to create HF API: {e}")))?;
        ...
        let config_path = repo
            .get("config.json")
            .map_err(|e| MnemoError::Router(format!("Failed to download config: {e}")))?;

        let tokenizer_path = repo
            .get("onnx/tokenizer.json")
            .map_err(|e| MnemoError::Router(format!("Failed to download tokenizer: {e}")))?;

        let weights_path = repo
            .get("model.safetensors")
            .map_err(|e| MnemoError::Router(format!("Failed to download weights: {e}")))?;

        let config_str = std::fs::read_to_string(&config_path)
            .map_err(|e| MnemoError::Router(format!("Failed to read config: {e}")))?;

        let model_config: ModelConfig = serde_json::from_str(&config_str)
            .map_err(|e| MnemoError::Router(format!("Failed to parse config: {e}")))?;
        ...
        let tokenizer = Tokenizer::from_file(&tokenizer_path)
            .map_err(|e| MnemoError::Router(format!("Failed to load tokenizer: {e}")))?;

        let vb = unsafe {
            VarBuilder::from_mmaped_safetensors(&[weights_path], DTYPE, &device)
                .map_err(|e| MnemoError::Router(format!("Failed to load weights: {e}")))?
        };

        let model = BertModel::load(vb.pp("bert"), &model_config.bert_config)
            .map_err(|e| MnemoError::Router(format!("Failed to load BERT model: {e}")))?;
        ...
        .map_err(|e| MnemoError::Router(format!("Failed to load classifier: {e}")))?;
    ...
    pub fn extract_entities(&self, text: &str) -> Result<Vec<Entity>, MnemoError> {
        let encoding = self
            .tokenizer
            .encode(text, true)
            .map_err(|e| MnemoError::Router(format!("Tokenization failed: {e}")))?;
        ...
        let token_ids_tensor = Tensor::new(token_ids, &self.device)
            .map_err(|e| MnemoError::Router(format!("Failed to create tensor: {e}")))?
            .unsqueeze(0)
            .map_err(|e| MnemoError::Router(format!("Failed to unsqueeze: {e}")))?;

        let token_type_ids = token_ids_tensor
            .zeros_like()
            .map_err(|e| MnemoError::Router(format!("Failed to create token_type_ids: {e}")))?;

        let hidden_states = self
            .model
            .forward(&token_ids_tensor, &token_type_ids, None)
            .map_err(|e| MnemoError::Router(format!("Model forward pass failed: {e}")))?;

        let logits = self
            .classifier
            .forward(&hidden_states)
            .map_err(|e| MnemoError::Router(format!("Classifier forward failed: {e}")))?;

        let logits = logits
            .squeeze(0)
            .map_err(|e| MnemoError::Router(format!("Failed to squeeze logits: {e}")))?;

        let probabilities = candle_nn::ops::softmax(&logits, 1)
            .map_err(|e| MnemoError::Router(format!("Softmax failed: {e}")))?;

        let predictions = logits
            .argmax(1)
            .map_err(|e| MnemoError::Router(format!("Argmax failed: {e}")))?
            .to_vec1::<u32>()
            .map_err(|e| MnemoError::Router(format!("Failed to convert predictions: {e}")))?;

        let probs_2d = probabilities
            .to_dtype(DType::F32)
            .map_err(|e| MnemoError::Router(format!("Failed to convert dtype: {e}")))?;
