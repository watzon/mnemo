//! Storage tier management for Mnemo
//!
...
impl<'a> TierManager<'a> {
    ...
    pub async fn migrate(&self, memory_id: Uuid, from: StorageTier, to: StorageTier) -> Result<()> {
        let memory = self
            ...
            .get(memory_id)
            .await?
            .ok_or_else(|| MnemoError::Memory(format!("Memory not found: {memory_id}")))?;

        if memory.tier != from {
            return Err(MnemoError::Memory(format!(
...
mod tests {
    ...
    mod tier_manager {
        ...
        async fn test_migrate_tier_mismatch_error() {
            ...
            assert!(matches!(err, MnemoError::Memory(_)));
        }
