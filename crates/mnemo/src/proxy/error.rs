...
pub fn handle_router_error(error: &MnemoError) -> PassthroughDecision {
    error!(
        error_type = "router",
        error_message = %error,
...
/// Strategy: Log the error and skip memory injection, allowing the
/// request to pass through without memory features.
pub fn handle_retrieval_error(error: &MnemoError) -> PassthroughDecision {
    error!(
...
pub fn handle_ingestion_error(error: &MnemoError) {
...
pub fn handle_request_error(error: &MnemoError) -> PassthroughDecision {
    error!(
        error_type = "request",
        error_message = %error,
...
pub fn handle_network_error(error: &MnemoError) -> PassthroughDecision {
    error!(
        error_type = "network",
        error_message = %error,
...
pub fn handle_proxy_error(error: ProxyError) -> PassthroughDecision {
    match error {
        ProxyError::Router(_) => {
            // Convert to MnemoError for handler
            let mnemo_err = MnemoError::Router(error.to_string());
            handle_router_error(&mnemo_err)
        }
        ProxyError::Retrieval(_) => {
            let mnemo_err = MnemoError::Memory(error.to_string());
            handle_retrieval_error(&mnemo_err)
        }
        ProxyError::Ingestion(_) => {
            let mnemo_err = MnemoError::Memory(error.to_string());
            handle_ingestion_error(&mnemo_err);
        ...
        ProxyError::Request(_) => {
            let mnemo_err = MnemoError::Proxy(error.to_string());
            handle_request_error(&mnemo_err)
        }
        ProxyError::Network(_) => {
            let mnemo_err = MnemoError::Proxy(error.to_string());
            handle_network_error(&mnemo_err)
        }
    }
}
...
pub fn with_error_handling<T, F>(
    operation: F,
    error_converter: impl FnOnce(MnemoError) -> ProxyError,
) -> Result<T, ProxyError>
where
    F: FnOnce() -> Result<T, MnemoError>,
{
...
mod tests {
    ...
    async fn test_handle_router_error_returns_skip_memory() {
        let error = MnemoError::Router("classification failed".to_string());
    ...
    async fn test_handle_retrieval_error_returns_skip_memory() {
        let error = MnemoError::Memory("database timeout".to_string());
    ...
    fn test_handle_ingestion_error_does_not_panic() {
        let error = MnemoError::Memory("insert failed".to_string());
    ...
    async fn test_handle_request_error_returns_bad_request() {
        let error = MnemoError::Proxy("missing field".to_string());
    ...
    async fn test_handle_network_error_returns_bad_gateway() {
        let error = MnemoError::Proxy("connection timeout".to_string());
    ...
    fn test_with_error_handling_failure() {
        let result = with_error_handling(
            || Err::<i32, _>(MnemoError::Router("fail".to_string())),
            |e| ProxyError::Router(e.to_string()),
        );
